...
for tweet in tweepy.Cursor(api.search, q=Query_to_search,lang='en').items():
    try:
        ...
        if (tweet.retweet == False) or (tweet.favorited == False):
            ...
            print(tweet.user.email)
            tweet.retweet()   # retweeting the particular tweet , (in your profile)
            tweet.favorite()  # adding to the favourite (the tweet .)
            api.add_list_member(screen_name = tweet.user.screen_name,owner_screen_name = myBot.screen_name,list_id = my_list.id)  # adding the member we find to the list
            print("Retweeted, favourite the tweet and added them into the list.!")
